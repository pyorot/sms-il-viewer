...
function navtop() {
  $("#navtop").html(`
    ...
    <label for="nt-3" onclick="return bodyPlayer(p)">Players</label>
  `)
}
...
function lbAggregate(s_, sort_=sortAggregate) {
  ...
  for (let [i,row] of table.entries()) {
    ...
    rowHTML[1] = `<td class="cell-a2 selectable" onclick="bodyPlayer(${row[1].p})">${row[1].name}</td>`
...
function lbLevel(l_) {
  ...
  for (let x of table) {
    ...
    html += `<tr>
      ...
      <td class="cell-l2 selectable" onclick="bodyPlayer(${x.p})">${data.players.names[x.p]}</td>
...
var sortPlayer = 0 // default sort column index
const sortMethodsPlayer = {
  0: (x,y) => {return x.l      - y.l     }, // level sort
  1: (x,y) => {return x.rank   - y.rank  }, // rank sort
  2: (x,y) => {return y.points - x.points}, // points sort
}
function bodyPlayer(p_, sort_=sortPlayer) {
  titlePlayer(p_, sort_)
  lbPlayer(p_, sort_)
  $("#navleft").css("display", "none")
}
function titlePlayer(p_, sort_=sortPlayer) { // p__ is the next level of dummy variable
  $("#title").html(`
    <select id="sel" name="sel" onchange="lbPlayer($('select#sel option').filter(':selected').val())">
...
function lbPlayer(p_, sort_=sortPlayer) {
  p = p_; sortPlayer = sort_ // update persistent global index
  let table = data.body.map(levelData => levelData[p_])
  table = table.filter(x => !!x.rank).sort(sortMethodsPlayer[sort_])
  let html = `<table><tr>
      <th class="cell-p1 selectable" onclick="lbPlayer(p,0)">level</th>
      <th class="cell-p2 selectable" onclick="lbPlayer(p,1)">rank</th>
      <th class="cell-p3 selectable" onclick="lbPlayer(p,2)">points</th>
